...
- `src/core_service_ipc/authorization.rs` owns peer identity and authorization; `client.rs` owns the CoreRuntime client; `error.rs` owns safe client error mapping; `ingress.rs` owns secure Runtime Bundle staging; `server.rs` owns dispatch and bounded lifecycle; `socket.rs` owns private socket setup and cleanup; `wire.rs` owns private protocol DTOs.
