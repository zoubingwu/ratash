...
- `src/runtime_bundle.rs` atomically stages private Runtime Generations, names the staged Core executable `ratash-mihomo`, and binds the Effective Configuration, bundled Mihomo executable, and local provider files to one verified manifest.
...
- `scripts/publish-cloudflare-r2.sh` publishes complete versioned macOS installer sets to the `ratash-releases` R2 bucket and advances the public latest pointer last; `packaging/release/cloudflare-r2.md` records the custom-domain, cache, and CI credential contract.
...
- `install.sh` is the fixed public lifecycle entry point for installing, updating, and uninstalling signed Ratash releases while preserving user Profile and rule data.
- `packaging/macos/` and `scripts/package-macos.sh` define the signed per-architecture installer payload, install the privileged service executable as `ratash-core-runtime` and the bundled Core as `ratash-mihomo`, and retain `io.ratash.core-runtime` as the stable LaunchDaemon label. They also own the pinned Mihomo and Geo-data artifacts and uninstaller without performing installation during development. `scripts/package-local-macos.sh` builds a personal `local-unsigned` package with the same payload and pinned resources using credential-free ad-hoc code identities. `scripts/validate-pinned-mihomo-geodata.sh` runs parse-only MMDB and DAT acceptance checks through pinned Mihomo without starting the Core data plane.
