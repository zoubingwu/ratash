...
fn bare_binary_prints_help_and_succeeds() {
    let output = Command::new(env!("CARGO_BIN_EXE_ratash"))
        .output()
        .expect("ratash should start");

    assert!(output.status.success());
    assert!(String::from_utf8_lossy(&output.stdout).contains("Usage: ratash [COMMAND]"));
...
fn version_flag_prints_the_package_version() {
    ...
    assert!(output.status.success());
    assert_eq!(String::from_utf8_lossy(&output.stdout), "ratash 0.1.1\n");
    assert!(output.stderr.is_empty());
}
...
fn help_points_to_the_agent_contract() {
    let output = Command::new(env!("CARGO_BIN_EXE_ratash"))
        .arg("help")
        .output()
        .expect("ratash help should start");

    assert!(output.status.success());
    assert!(
        String::from_utf8_lossy(&output.stdout).contains("ratash help agent"),
        "{}",
...
fn agent_help_contains_the_safe_rule_workflow() {
    let output = Command::new(env!("CARGO_BIN_EXE_ratash"))
        .args(["help", "agent"])
        .output()
        .expect("ratash help agent should start");
...
fn json_operation_failure_uses_stderr_and_the_supervisor_exit_code() {
    let output = Command::new(env!("CARGO_BIN_EXE_ratash"))
        .args(["status", "--json"])
        .output()
        .expect("ratash status --json should start");
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "error": {
                ...
                "message": "The Ratash Supervisor IPC endpoint is unavailable",
                "retryable": true
            }
        })
    );
...
fn invalid_subscription_url_is_redacted_from_human_and_json_errors() {
    ...
    let human = Command::new(env!("CARGO_BIN_EXE_ratash"))
    ...
    let json = Command::new(env!("CARGO_BIN_EXE_ratash"))
