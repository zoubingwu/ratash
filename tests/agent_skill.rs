use hopash::cli::render_agent_help;

const SKILL: &str = include_str!("../skills/hopash/SKILL.md");

#[test]
fn packaged_skill_uses_the_live_agent_help_as_authority() {
    assert!(SKILL.contains("hopash help agent"));
    assert!(SKILL.contains("Use the installed `hopash` executable as the only product interface"));
}
